...
use crate::ui::{ACCENT, HEADER_FG, MUTED};

pub fn render_scanning(frame: &mut Frame, area: Rect, scanning_frame: usize) {
...
pub fn render_picker(frame: &mut Frame, area: Rect, state: &FilePickerState) {
    ...
    let query = Paragraph::new(format!("Query: {}", state.query)).block(
        Block::default()
    ...
    let footer =
        Paragraph::new("Type to filter | Up/Down to navigate | Enter to continue | Esc/q to quit")
